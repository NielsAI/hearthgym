...
st.header("Action Counts")
    
with st.expander("Action Counts per player", expanded=True):
    st.pyplot(st.session_state.analyzer.fig_combo_distribution_lengths)
    
with st.expander("Action Counts Grid", expanded=True):
    st.pyplot(st.session_state.analyzer.fig_combo_distribution_grid)